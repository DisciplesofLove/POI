...
class EdgeNode(InferenceNode):
    ...
    async def _execute_task(self, task_id: str):
        ...
        try:
            ...
            if task.get('isFree'):
                # Process through IOTA for free tier
                message_id = await self.iota_batch_handler.add_request(
                    task['modelId'],
                    input_data
                )
                result = await self.iota_handler.receive_inference_result(
                    task['modelId'],
                    message_id
                )
            else:
                # Standard paid inference
                result = await self.execute_inference(
                    task['modelId'],
                    input_data,
                    task.get('userId')  # Pass user ID for predictive loading
                )
