...
import SovereignRPCABI from '../contracts/SovereignRPC.json';
import { IIOTAValidator__factory } from '../typechain/factories/IIOTAValidator__factory';

const SOVEREIGN_RPC_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_RPC_ADDRESS;
const IOTA_VALIDATOR_ADDRESS = process.env.NEXT_PUBLIC_IOTA_VALIDATOR_ADDRESS;
...
export async function getProvider(): Promise<ethers.providers.Web3Provider> {
    ...
    const rpcEndpoint = await rpcManager.getBestNode();
