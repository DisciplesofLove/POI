...
async function registerModel(web3, account, modelData) {
    try {
        const marketplace = new web3.eth.Contract(ModelMarketplaceABI, MARKETPLACE_ADDRESS);
        const tx = await marketplace.methods.registerModel(
            web3.utils.keccak256(modelData.id),
            modelData.metadata,
            web3.utils.toWei(modelData.price.toString(), 'ether')
        ).send({ from: account });
        return tx;
    } catch (error) {
...
async function useModel(web3, account, modelId, executionId) {
    try {
        const marketplace = new web3.eth.Contract(ModelMarketplaceABI, MARKETPLACE_ADDRESS);
        const tx = await marketplace.methods.useModel(
            web3.utils.keccak256(modelId),
            web3.utils.keccak256(executionId)
        ).send({ from: account });
        return tx;
    } catch (error) {
